...
export default function SplashScreen({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div className={`fixed inset-0 z-[9999] flex items-center justify-center bg-black transition-opacity duration-700 ${phase === "fadeout" ? "opacity-0" : "opacity-100"}`}>
      ...
      <div className="absolute top-0 bottom-0 w-[3px] blur-sm"
        style={{
      ...
      <div className="absolute top-0 bottom-0 w-[120px] blur-lg"
        style={{
      ...
      <div className={`relative z-10 transition-all duration-700 ${
        ...
        <div className={`absolute inset-0 rounded-full transition-all duration-700 ${phase === "glow" ? "opacity-100" : "opacity-0"}`}
          style={{ boxShadow: "0 0 80px 40px rgba(168,85,247,0.3)" }} />
        <Image src="/logo.png" alt="MigoPlay" width={420} height={140}
          className={`object-contain transition-all duration-700 ${
            phase === "glow" ? "drop-shadow-[0_0_40px_rgba(168,85,247,0.9)]" : "drop-shadow-[0_0_15px_rgba(168,85,247,0.4)]"
          }`} priority />
